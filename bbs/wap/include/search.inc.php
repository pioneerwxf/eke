...
/*
	...
	$Id: search.inc.php 9806 2007-08-15 06:04:37Z cnteacher $
*/
...
if($do != 'submit') {
	...
	if(isset($searchid)) {
		...
		$query = $db->query("SELECT COUNT(*) FROM  {$tablepre}threads WHERE tid IN ($index[tids]) AND displayorder>='0'");
		$searchnum = $db->result($query, 0);
		if($searchnum) {
		...
		$srchuid = isset($srchuid) ? intval($srchuid) : 0;
